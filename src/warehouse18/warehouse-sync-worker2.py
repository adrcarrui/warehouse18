...
MARK_MOVEMENT_SYNCING_SQL = text(
    ...
    WHERE id = :movement_id
      AND mysim_sync_status IN ('queued', 'error', 'syncing')
    """
)

MARK_MOVEMENT_OK_SQL = text(
    """
    UPDATE movements
    SET
        mysim_sync_status = 'ok',
        mysim_sync_error = NULL,
        mysim_synced_at = now()
    WHERE id = :movement_id
    """
...
def process_one_job(outbox_row: dict) -> None:
    ...
    try:
        print(
            f"[outbox] processing | outbox_id={outbox_id} movement_id={movement_id} "
        ...
        if movement.mysim_sync_status != "ok":
            raise RuntimeError(
                f"sync_movement_to_mysim finished without ok status | "
                f"movement_id={movement_id} status={movement.mysim_sync_status} "
                f"error={movement.mysim_sync_error}"
            )

        db.execute(
            MARK_MOVEMENT_OK_SQL,
            {
                "movement_id": movement_id,
            },
        )
...
def main() -> None:
    ...
    while True:
        ...
        try:
            rows = claim_jobs(db)
            if rows:
                print(f"[outbox] claimed {len(rows)} job(s)")
        except Exception:
            db.rollback()
